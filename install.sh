...
# Checks
[[ ! $(id -u) -eq 0 ]] && eecho "Execute this script with sudo: \"sudo -E ./$SCRIPT_NAME\".";
...
{ [ -z "$SLACK_URL" ] || [ -z "$TELEGRAM_GROUP_ID" ] || [ -z "$TELEGRAM_BOT_TOKEN" ]; } && eecho "One or more core variables are empty! Check SLACK_URL, TELEGRAM_GROUP_ID, TELEGRAM_BOT_TOKEN env variables, all variables must have a value!"
